...
include('config.php');
    ...
    $sql1 = "SELECT * FROM `bc_data` WHERE `id`='$bc_id'";
    ...
    if (mysqli_num_rows($result)>0) {
        ...
        <div id='container'>
        <div class='row my-5'>
        <h3 class='h3 text-center'>Upload Basic Concepts</h3>
        </div>
        <div class='row'>
        <form action='' method='post'>
            <div class='mb-3'>
                <label for='topic' class='form-label'>Topic</label>
                <input type='text' name='topic' id='topic' class='form-control' value='{$rows['bc_topic']}' placeholder='enter your topic'
                    aria-describedby='helpId' />
            </div>
            <div class='mb-3'>
                <label for='editor' class='form-label'>Contents</label>
                <textarea class='form-control' name='editor' id='editor' rows='7'>{$rows['bc_content']}</textarea>
            </div>
...
if (isset($_POST["submit"])) {
    ...
    // Use prepared statement to avoid SQL injection
    $sql2 = "INSERT INTO `bc_data`(`bc_topic`, `bc_content`) VALUES (?, ?)";
